...
|---------------|-------------|--------------|
| Go | Язык программирования | Backend | Высокая производительность, встроенная поддержка многопоточности, широкая экосистема и поддержка различных библиотек, хорошо подходит для микросервисов |
...
| PostgreSQL | СУБД | хранение данных |  |
| MongoDB | СУБД | хранение данных |  |
| ClickHouse | СУБД | хранение высокочастотных данных и дальнейший анализ | |
| Redis | СУБД | хранение данных, сессий пользователя |  |
| Kafka | СУБД | хранение высокочастотных данных для рекомендательной системы и дальнейший анализ|  |
| ElasticSearch | СУБД | Поиск |  |
| S3 | СУБД | хранение данных |  |
| Kafka | СУБД | сбор и анализ пользовательских действий |  |
| CDN | серверная технология | высоконагрженные приложения | значительно ускоряет доставку данных до клиента и снижает нагрузку на датацентры |
| VRRP | алгоритм |  |  |
...
| Kubernetes | система оркестрации | управление экземплярами приложения, их автоматическая перезагрузка и перераспределение | единственное удобное и широкоиспользуемое решение |
