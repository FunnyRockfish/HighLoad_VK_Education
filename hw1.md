...
(схема)
![локальная балансировка](https://github.com/user-attachments/assets/13887cb7-d14c-41bd-bce1-6140718ce089)
...
### Kubernetes:
Мы будем использовать систему оркестрации **Kubernetes**, чтобы не запускать экземпляры приложения вручную на серверах (bare metal). Kubernetes берет на себя **service discovery** — управляет сервисами и динамически отслеживает список активных экземпляров с помощью механизма **Service** и **Endpoints**.
