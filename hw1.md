...
- **MongoDB**: Используется для хранения данных с нефиксированной структурой, где важна гибкость. Хорошо подходит для контента с JSON-полями, такими как видео и статьи.
- **PostgreSQL**: Реляционная база данных для данных, требующих строгой целостности и структурированных связей (например, данные пользователей и связи с каналами).
- **ClickHouse**: Высокопроизводительная аналитическая база данных для больших объемов данных и быстрых агрегатов (например, статистика и логи).
- **Redis**: In-memory хранилище для временных данных (например, сессии), где важна скорость чтения и записи.
...
|-------------------------|-------------------------------------------------------------------------------|----------------|
| `video`                 | Содержит информацию о видео (заголовок, описание, автор, форматы, метаданные). | хеширование по channel_id     | 
| `article`               | Хранит статьи и связанные с ними данные (заголовок, текст, теги, изображения). | хеширование по channel_id      | 
| `content_comments`      | Комментарии к контенту, включая ответы и статус комментария.                  | хеширование по content_id   |
| `user`                  | Данные пользователей (имя, email, аватар, статус и информация о регистрации).  | хеширование по user_id  | 
| `session`               | Сессии пользователей (для трекинга авторизаций).                              | -       | 
| `channel`               | Информация о каналах, включая описание и количество подписчиков.              | хеширование по channel_id  | 
| `channel_subscriber`    | Связи между пользователями и каналами (подписки).                             | хеширование по user_id  | 
| `content_statistic`     | Статистика взаимодействий с контентом (лайки, просмотры, комментарии).        | хеширование по channel_id  |
| `user_statistic`        | Статистика просмотров пользователя (история видео и статей).                  |  хеширование по user_id  | 
| `favorite_item`         | Избранные элементы пользователя (статьи и видео).                             |  хеширование по user_id  | 
| `search_logs`           | Логи поиска пользователей.                                                    | хеширование по user_id  | 
