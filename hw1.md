...
### 3. **Показывать стандартный/популярный контент вместо персонализированного**:
   - **Стандартные рекомендации**: при проблемах с системой персонализации показывать пользователю стандартную ленту с популярными публикациями за последние несколько дней, чтобы поддерживать интерес. За это отвечает сервис RecommendationFallback: будет возвращать контент, близкий пользователю - сначала смотрим в избранное, получаем список авторов эти карточек, и показываем пользователю последние и самые популярные и последние карточки контента этого автора. Если же список израбнного пуст, или слишком мал, то идём в сервис ContentFallbackService, он отдаст нам популярные карточки.
...
### 7. **Отложенные операции**:
   - **Очередь для лайков и сохранений**: если сервис, обрабатывающий лайки, подписки и добавления в избранное, перегружен, можно временно складывать эти действия в очередь и синхронизировать их позже, показывая пользователю уведомление, что операция выполнена. Для этого используем Kafka.
...
### Пример сценария:
Если сервис рекомендаций выходит из строя, платформа переключается на показ фиксированного набора популярных публикаций, подгружаемых из кэша или получаемых на основе избранного и подписок пользователя. Интерфейс при этом может оставаться таким же, чтобы пользователь не заметил перехода на стандартный набор данных.
...
![Схема_проекта_patch](https://github.com/user-attachments/assets/3be897c4-a056-4d6f-8adc-25e79734de09)  
...
| **Сервис**          | **Пиковая нагрузка в RPS** | **Пиковая нагрузка в GBit/s** | **RAM на экземпляр** | **CPU на экземпляр** | **Количество серверов** | **Общая RAM** | **Общее количество подов** | **Стоимость аренды сервера (USD/мес)** | **Стоимость покупки сервера (USD)** |
|---------------------|----------------------------|-------------------------------|-----------------------|-----------------------|--------------------------|----------------|-----------------------------|----------------------------------------|-------------------------------------|
| **auth**            | 20                         | < 0.1                         | 2 ГБ                  | 2 CPU                 | 8                        | 16 ГБ           | 16                          | 400 USD                                 | 6 400 USD                            |
| **profile**         | 2                          | < 0.1                         | 2 ГБ                  | 2 CPU                 | 8                        | 16 ГБ           | 16                          | 400 USD                                 | 6 400 USD                            |
| **content**         | 24 900                     | 947                           | 8 ГБ                  | 4 CPU                 | 52                       | 5 ТБ            | 260                         | 2 600 USD                               | 41 600 USD                           |
| **file**            | 24 100                     | 965                           | 64 ГБ                 | 8 CPU                 | 12                       | 768 ГБ          | 36                          | 1 200 USD                               | 19 200 USD                           |
| **recommend**       | 25 000                     | 5                             | 8 ГБ                  | 8 CPU                 | 20                       | 160 ГБ          | 80                          | 2 000 USD                               | 32 000 USD                           |
| **statistic**       | 1 000                      | 1                             | 1 ГБ                  | 2 CPU                 | 12                       | 12 ГБ           | 24                          | 600 USD                                 | 9 600 USD                            |
| **contentControl**  | 100                        | < 0.1                         | 1 ГБ                  | 2 CPU                 | 8                        | 8 ГБ            | 16                          | 400 USD                                 | 6 400 USD                            |
| **comment**         | 3                          | < 0.1                         | 2 ГБ                  | 2 CPU                 | 8                        | 16 ГБ           | 16                          | 400 USD                                 | 6 400 USD                            |
| **search**          | 250                        | 12                            | 4 ГБ                  | 4 CPU                 | 12                       | 48 ГБ           | 24                          | 1 200 USD                               | 19 200 USD                           |
| **favorite**        | 150                        | 4                             | 2 ГБ                  | 2 CPU                 | 8                        | 16 ГБ           | 16                          | 400 USD                                 | 6 400 USD                            |
| **channel**         | 2                          | < 0.1                         | 2 ГБ                  | 2 CPU                 | 8                        | 16 ГБ           | 16                          | 400 USD                                 | 6 400 USD                            |

_При покупке сервера вместо аренды окупаемость будет достигнута через 2 года (3686400 USD / 155000 USD)_

### Таблица 11.3: Графические процессоры (GPU) для ML в ДЗЕНе

| **Модель GPU**   | **Стоимость (USD)** | **Количество** | **Назначение**                                           | **Описание**                                                   |
|------------------|---------------------|-----------------|-----------------------------------------------------------|----------------------------------------------------------------|
| **NVIDIA T4**    | ~2 500$              | 16               | Машинное обучение (ALS, CatBoost), NLP (BERT, T5), TTS    | Эффективны для инференса и обучения моделей средней сложности  |
| **NVIDIA A100**  | ~10 000$             | 12               | Обучение крупных моделей NLP и TTS                        | Высокая производительность для обучения больших моделей        |
| **NVIDIA V100**  | ~8 000$              | 4               | Специализированные задачи машинного обучения и обработки данных | Альтернативный вариант для высокопроизводительных задач     |
